...
router.get("/fetch-settings", async (req, res) => {
    try {
        ...
        const settings = await settingsModel.findOne({ sellerID }).select("shippingGap deliveryGap")
...
router.put("/save-changes", async (req, res) => {
    try {
        const { sellerID } = req.query
        const { shippingGap, deliveryGap } = req.body

        if (!sellerID) return res.status(400).json({ message: "sellerID is required" });

        await settingsModel.findOneAndUpdate({ sellerID }, { shippingGap, deliveryGap }, { new: true })
