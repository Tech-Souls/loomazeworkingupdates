                            ...
                            <a key={i} href={slide.ctaLink || (isCustomDomain ? "/" : `/brand/${settings.brandSlug}`)} target="_blank" className="relative block hero-slider-media cursor-pointer">
                                ...
                                {(slide.title || slide.subtitle) &&
                                    <div className="w-full p-4 absolute left-1/2 -translate-x-1/2 sm:translate-x-0 sm:left-56 top-1/2 -translate-y-1/2 flex flex-col justify-center items-start">
                                        {slide.title && <h2 className="text-white text-2xl md:text-4xl font-bold">{slide.title}</h2>}
                                        {slide.subtitle && <p className="text-white max-w-xs text-sm my-4">{slide.subtitle}</p>}
                                        {(slide.title || slide.subtitle) &&
                                            ...
                                            >
                                                <span className="block px-10 sm:px-16 py-2 sm:py-3 bg-white hover:bg-white text-black hover:text-[#202020] transition-all duration-500 ease-out">Shop Now</span>
