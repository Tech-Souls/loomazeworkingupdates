...
const NextArrow = ({ onClick }) => (
    <button
        className="absolute top-1/2 right-3 z-10 -translate-y-1/2 p-2 text-white" onClick={onClick}
    >
...
const PrevArrow = ({ onClick }) => (
    <button
        className="absolute left-3 top-1/2 z-10 -translate-y-1/2 text-white"
        onClick={onClick}
    >
        <ChevronLeft size={35} />
    </button>
...
export default function PlatformHeroThree({ settings, isCustomDomain }) {
    ...
    return (
        <section className="relative w-full overflow-hidden hero-two-slider px-2.5 md:px-4 lg:px-10">
            <Slider {...storeSettings} className="[&_.slick-list]:overflow-hidden [&_.slick-list]:rounded-2xl">
                {slides.map((slide, i) => {
                    const heroSliderDataExists = settings?.content?.heroSlider?.length > 0 ? true : false
                    return (
                        <a key={i} href={slide.ctaLink || (isCustomDomain ? "/" : `/brand/${settings.brandSlug}`)} target="_blank" className="block rounded-2xl overflow-hidden">
                            <img
                                src={!heroSliderDataExists ? slide.image : `${slide.image}`}
                                alt={`banner-${i + 1}`}
                                loading="lazy"
                                className="mx-auto h-[30vh] sm:h-[50vh] md:h-[70vh] lg:h-[85vh]
                         w-full object-cover transition-transform duration-[1000ms]
                         ease-[cubic-bezier(0.7,0,0.3,1)] hover:scale-110"
                            />
