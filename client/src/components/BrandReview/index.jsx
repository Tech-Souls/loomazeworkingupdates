...
export default function BrandReview({ storeSettings }) {
    const demoReviews = [
  {
    ...
    comment: "Amazing product! Really loved the quality and fast delivery.",
    rating: 5,
    createdAt: "2026-02-08T12:30:00Z",
  },
  {
    userID: { username: "JaneSmith" },
    comment: "Good service, but the packaging could be improved.",
    rating: 4,
    createdAt: "2026-02-07T09:15:00Z",
  },
  {
    userID: { username: "AlexR" },
    comment: "Average experience. Product is okay, but shipping was delayed.",
    rating: 3,
    createdAt: "2026-02-06T14:45:00Z",
  },
  {
    userID: { username: "LisaM" },
    comment: "Not satisfied with the product quality. Would not recommend.",
    rating: 2,
    createdAt: "2026-02-05T10:00:00Z",
  },
  {
    userID: { username: "MikeB" },
    comment: "Terrible experience! Package arrived damaged and late.",
    rating: 1,
    ...
    const fetchRecentReviews = () => {
        ...
        axios
            ...
            .then((res) => {
                if (res.status === 200 && res.data?.reviews?.length > 0) setReviews(res.data?.reviews);
            })
        ...
        <div className="w-full main-container pt-12 sm:pt-18 md:pt-26 relative">
            ...
            {reviews.length > 0 ? (
                <div className="relative px-8 sm:px-4">
                    ...
                    {/* <button
                        ref={prevRef}
                        className="absolute left-2 lg:-left-12 top-1/2 -translate-y-1/2 bg-[var(--pr)] text-white p-2 md:p-3 shadow-lg z-[9999] hover:bg-[var(--pr)]/70 transition-all duration-300 ease-linear hover:border border-gray-700"
                    >
                    ...
                    {/* <button
                        ref={nextRef}
                        className="absolute right-2 lg:-right-12 top-1/2 -translate-y-1/2 bg-[var(--pr)] text-white p-2 md:p-3 shadow-lg z-[9999] hover:bg-[var(--pr)]/70 transition-all duration-300 ease-linear hover:border border-gray-700"
                    >
