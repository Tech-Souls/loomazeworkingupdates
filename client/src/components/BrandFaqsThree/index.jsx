...
export default function BrandFaqsThree({ settings }) {
    ...
    return (
        <section className="relative w-full main-container py-16 px-4 sm:px-10 !my-20">
            <div className="absolute inset-0 bg-gradient-to-br from-[var(--pr)]/5 via-[var(--pr)]/3 to-transparent pointer-events-none rounded-[30px]" />

            <div className="relative grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
                <div className="space-y-6 text-center md:text-left flex flex-col items-center md:items-start">
                    <h2 className="text-3xl sm:text-4xl font-extrabold text-[var(--text)] leading-tight">
                        Got questions? <br />
                    ...
                    <button className="flex items-center gap-2 bg-[var(--pr)] text-white px-6 py-3 rounded-full text-sm font-semibold hover:bg-[var(--pr)]/10 hover:text-gray-800 hover:border border-gray-700 hover:gap-3 transition-all duration-300">
                    ...
                    {loading ?
                        ...
                        : faqs.length > 0 ?
                            faqs.map((faq, index) => (
                                <div
                                    key={faq._id}
                                    className={`rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300 ${openIndex === index ? " border-[var(--pr)]/40" : "bg-[#fafafa]"}`}
                                >
                                    <button
