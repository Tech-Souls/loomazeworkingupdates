...
export default function PlatformHeader({ settings, isCustomDomain, themeType }) {
    ...
    useEffect(() => {
        ...
        const fetchMainMenuItems = async () => {
            try {
                ...
                const res = await axios.get(`${import.meta.env.VITE_HOST}/platform/home/fetch-main-menu`, {
                    params: {
    ...
    // ─── Cart length update ───────────────────────────
    useEffect(() => setCartLength(user?.cart?.length || 0), [user?.cart])
    ...
    const buildNavPath = (url) => isCustomDomain ? `/${url}` : `/brand/${settings?.brandSlug}${url}`
        ...
        <>
            ...
            >
                    ...
                    <div className="hidden lg:flex items-center gap-6">
                        {loadingMainMenuItems ? (
                            ...
                            mainMenuItems.map(item => (
                                <button
                                    key={item._id}
                                    onClick={() => navigate(buildNavPath(item.url))}
                ...
                <div className="hidden lg:flex items-center gap-4">
                    ...
                    <button onClick={() => navigate(buildNavPath('/customer/favourites'))}>
                        <Heart size={20} className={`transition-colors duration-300 ${textColour}`} />
                    </button>

                    <button className="relative" onClick={() => handleAuthNavigation(buildNavPath('/cart'))}>
            ...
            {isLuxury && (
                <div className="relative h-screen w-full overflow-hidden">
                    ...
                    <div className="absolute bottom-24 left-6 md:left-20 text-white max-w-xl z-10">
                        ...
                        <button
                            onClick={() => navigate(buildNavPath('/pages/products'))}
