...
export default function PlatformHeroDouble({ settings, isCustomDomain }) {
    const dummy = { image: heroShoesImg, ctaLink: isCustomDomain ? `/` : `/brand/${settings?.brandSlug}`, };

    const [slide, setSlide] = useState(dummy);

    useEffect(() => {
        if (settings?.content?.heroSlider2?.length > 0) {
            const hero = settings.content.heroSlider2[0];
            setSlide({
                ...hero,
                image: `${hero.image}`,
            });
    ...
    return (
        <section
            className="relative w-full h-[30vh] sm:h-[50vh] md:h-[70vh] lg:h-[80vh] flex items-center justify-end overflow-hidden mt-8"
            style={{
