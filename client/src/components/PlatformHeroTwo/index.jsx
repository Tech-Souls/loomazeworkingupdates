...
const NextArrow = ({ onClick }) => (
    <button
        className="absolute top-1/2 right-3 z-10 -translate-y-1/2 bg-white/70 hover:bg-white rounded-full p-2 shadow-md 
               transition-all duration-300"  onClick={onClick}
    >
...
const PrevArrow = ({ onClick }) => (
    <button
        className="absolute left-3 top-1/2 z-10 -translate-y-1/2 
               bg-white/70 hover:bg-white rounded-full p-2 shadow-md 
...
export default function PlatformHeroTwo({ settings, isCustomDomain }) {
        ...
        <section className="relative w-full overflow-hidden hero-two-slider">
            <Slider {...sliderSettings}>
                {slides.map((slide, i) => {
                    ...
                    return (
                        <a key={i} href={slide.ctaLink || (isCustomDomain ? "/" : `/brand/${settings.brandSlug}`)} target="_blank" className="block">
                            <img
