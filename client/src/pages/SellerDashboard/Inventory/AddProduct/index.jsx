...
const initialState = {
    title: "", description: "", category: "", subcategory: "", sku: "",
    price: 0, comparedPrice: 0, stock: 0,
    weight: 0, weightUnit: "kg", taxable: false, isFeatured: false,
    sizeChart: null, tags: [], paymentModes: ["cod", "online"], boughtTogether: [],
}
...
export default function AddProduct() {
    ...
    const fetchCategories = () => {
        axios.get(`${import.meta.env.VITE_HOST}/seller/add-product/fetch-categories?sellerID=${user._id}`)
            .then(res => {
    ...
    const fetchSizeCharts = () => {
        axios.get(`${import.meta.env.VITE_HOST}/seller/add-product/fetch-size-charts?sellerID=${user._id}`)
            .then(res => {
                const { status, data } = res
                if (status === 200) {
    ...
    useEffect(() => {
        ...
        const delayDebounce = setTimeout(() => {
            if (search.trim().length > 1) {
                axios.get(`${import.meta.env.VITE_HOST}/seller/add-product/frequent-bought-search?sellerID=${user._id}&q=${search}`)
    ...
    const handleAddProduct = () => {
        const { title, description, category, subcategory, sku, price, comparedPrice, taxable,
            isFeatured, stock, weight, weightUnit, sizeChart, tags, paymentModes, boughtTogether
        } = state

        if (!title || !mainImage) return window.toastify("Title and Image is required!", "info")
        if (price <= 0) return window.toastify("Enter Price of the product!", "info")
        if (stock < 0) return window.toastify("Stock cannot be negative!", "info")

        const formData = new FormData();
        formData.append("sellerID", user._id);
        formData.append("brandSlug", user.brandSlug);
        ...
        setLoading(true);
        axios.post(`${import.meta.env.VITE_HOST}/seller/add-product/create`, formData, {
    ...
    return (
        <>
            <div className='bg-[#fafafa] px-3 sm:px-5 md:px-8 py-2.5 sm:py-4'>
                ...
                <p className='text-[12px] sm:text-[13px] text-gray-900'>Add a new product in your inventory. Enter product specific details for better experience.</p>
            </div>

            <div className='seller-container !pb-20'>
                <div className='flex justify-between items-center'>
                    <p className='w-fit text-[12px] font-bold'><span className='text-gray-400'>Seller</span> / Inventory / Add Product</p>
                ...
                <div className='flex justify-between gap-5 mt-4'>
                    <div className='flex flex-col flex-1 gap-5'>
                        ...
                        <div className='flex flex-col p-4 sm:p-6 bg-white shadow'>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className='flex items-center gap-2 mt-3'>
                                                    {values.map((v, i) => (
                                                        <span key={i} className='text-xs bg-gray-200 text-gray-700 px-3.5 py-1 border border-gray-200 rounded-lg'>{v}</span>
                                                    ))}
                                                </div>
                                            </div>
                                        )
                            ...
                            >
                                <BiPlusCircle size={18} /> {options.length > 0 ? "Add another option" : "Add options like size or color"}
                            </button>
                        ...
                        <div className='flex flex-col gap-4 p-4 sm:p-6 bg-white shadow'>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="mt-2 w-full bg-white border border-gray-300 !p-3 !rounded-none text-sm"
                                />
                    ...
                    <div className='flex flex-col gap-6 w-full max-w-[400px]'>
                        <div className='p-4 sm:p-6 bg-white shadow'>
                            ...
                            <div className='group relative w-full h-[300px] bg-white border-2 border-dashed border-gray-300 mt-3 transition-all duration-200 ease-linear hover:border-[var(--secondary)]'>
                                ...
                                <div className='flex flex-col justify-center items-center absolute inset-0 p-3 text-gray-500 cursor-pointer transition-all duration-200 ease-linear group-hover:text-[var(--secondary)]' onClick={() => mainImageRef.current.click()}>
                                    {
                                        !mainImage ?
                                            ...
                                            <div className='h-full'>
                                                <img src={mainImagePreview} alt="product-main-image" className='max-h-full' />
                                                <div className="absolute top-3 right-5">
                                                    <button className='text-lg text-gray-900'
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            setMainImage(null);
                                                            setMainImagePreview(null);
