...
export default function ReferAndEarnSeller() {
  ...
  const referralLink = user
    ? `${window.location.origin}/auth/seller/signup?ref=${user.referralCode}`
    : "";
  const fetchStats = useCallback(async () => {
    if (!token) return;
    setLoading(true);
    try {
      const res = await axios.get(`${window.api}/referrals/seller`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      console.log("this is seller data: :", res.data);
      setData(res.data);
    } catch (err) {
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-6xl mx-auto space-y-6">
        <div>
          ...
          <Text type="secondary">
            Earn commissions for every seller you refer.
          </Text>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              <button
                ...
                disabled={(data?.totalEarned ?? 0) - (data?.pending ?? 0) <= 0}
                className="
    w-full
        ...
        >
          <Form
            ...
            onFinish={async (values) => {
              console.log(values);
              if(values.amount > ((data?.totalEarned ?? 0) - (data?.pending ?? 0))) {
              ...
              try {
                ...
                const res = await axios.post(
                  `${window.api}/withdraw/seller/create`,
                ...
                navigate("/seller/refer-and-earn/withdraws");
              } catch (err) {
                console.error(err);
