...
export default function Sellers() {
  ...
  const fetchSellers = (page) => {
    setLoading(true);
    axios
      .get(`${import.meta.env.VITE_HOST}/admin/sellers/list/all?page=${page}`)
      .then((res) => {
        // console.log('Sellers fetch data',res.data.sellers[0].brandName);
  ...
  const fetchSearchedSellers = (text, page) => {
    ...
    axios
      .get(
        `${import.meta.env.VITE_HOST}/admin/sellers/list/search?searchText=${text}&page=${page}`,
      )
      .then((res) => {
        if (res.status === 200) {
  ...
  const handleUpdateSeller = async (ans) => {
    ...
    try {
      const res = await axios.put(
        `${import.meta.env.VITE_HOST}/admin/sellers/list/update-status/${selectedSeller._id}`,
        { status: ans },
      );
      if (res.status === 202) {
  ...
  const handleDeleteSeller = async (id) => {
    ...
    try {
      const res = await axios.delete(
        `${import.meta.env.VITE_HOST}/admin/sellers/list/delete/${id}`,
      );
      if (res.status === 203) {
    ...
    <>
      <div className="bg-[#fafafa] px-3 sm:px-5 md:px-8 py-2.5 sm:py-4">
        ...
        <p className="text-[12px] sm:text-[13px] text-gray-900">
          View and manage all registered sellers on the platform.
        </p>
      </div>
      ...
      <div className="admin-container">
        <div className="flex justify-between items-center gap-5">
          <p className="w-fit text-[12px] font-bold">
            <span className="text-gray-400">Admin</span> / Seller List
          </p>
          ...
          <div className="flex flex-1 justify-end items-center gap-2.5">
            <input
              ...
              className="w-full max-w-[250px] text-sm bg-white !px-4 !py-2 !rounded-none"
              onChange={(e) => setSearchText(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && handleSearch()}
            />
            <button
              className="text-sm text-white bg-[var(--secondary)] px-6 py-2 transition-all duration-150 ease-linear hover:bg-[var(--secondary)]/70"
              ...
              {loading ? (
                ...
                sellers.map((seller) => (
                  <tr className="bg-white border-b border-gray-200">
                    ...
                    <td>
                      <a
                        href={`${storeUrl}/brand/${seller?.brandSlug}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="hover:underline"
                      >
                        {`${storeUrl}/brand/${seller?.brandSlug}`}
                      </a>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {dayjs(seller.createdAt).format("DD-MM-YYYY HH:mm")}
                    </td>
                    <td className="px-6 py-4 capitalize">
                      <span
                        className={`text-[10px] px-2 py-1 font-bold ${
                          seller.status === "pending"
                            ? "text-[#d4d407] bg-[#f6fac6]"
                            : seller.status === "approved"
                              ? "text-[#0ec520] bg-[#d1ffd5]"
                              : seller.status === "rejected"
                                ? "text-[#e63636] bg-[#ffe4e4]"
        ...
        {/* Update Modal */}
        <div
          className={`flex flex-col justify-between fixed top-0 left-0 w-full bg-white h-screen transition-all duration-300 ease-linear ${showUpdateModal ? "visible opacity-100 scale-100" : "invisible opacity-0 scale-75"}`}
        >
          ...
          <div className="w-full max-w-7xl mx-auto flex-1 p-6 overflow-y-auto">
            {selectedSeller?.status === "pending" && (
              <p className="text-sm text-orange-400 bg-orange-50 px-2.5 py-1 w-fit">
                Review the seller application for account approval
              </p>
            )}
            ...
            <div className="flex flex-col lg:flex-row justify-between gap-12 mt-8">
              <div className="text-gray-800 flex flex-1 flex-col gap-3 h-fit bg-[#f1f5f9c9] p-6">
                <p>
                  ...
                  <span className="inline-block px-3 py-0.5 bg-green-100 text-green-500">
          ...
          <div className="flex justify-end gap-3 p-6">
            ...
            <button
              className="px-4 py-1.5 bg-[#f78330] text-white"
