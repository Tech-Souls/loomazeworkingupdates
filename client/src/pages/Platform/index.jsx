...
import PlatformPremiumFooter from "../../components/PlatformPremiumFooter";
import PlatformToolFooter from "../../components/PlatformToolFooter";
...
import PlatformPremiumHeader from "../../components/PlatformPremiumHeader";
...
export default function Platform({ isCustomDomain }) {
  ...
  const fetchSellerSettings = () => {
    ...
    axios
      .get(
        `${import.meta.env.VITE_HOST}/platform/seller-settings/get?${queryParams}`,
      )
      .then((res) => {
        if (res.status === 200) {
      ...
      <div className="flex items-center justify-center min-h-screen text-center">
        <div>
          ...
          <p className="text-gray-600">
            This store doesn't exist or is not configured yet.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <>
      ...
      {currentTheme === "premium" ? (
        <PlatformPremiumHeader 
        settings={settings}
          isCustomDomain={isCustomDomain}
          themeType={currentTheme}
        />
      ) : currentTheme === "tools" ? (
        <PlatformToolHeader settings={settings}
          isCustomDomain={isCustomDomain}
          themeType={currentTheme}  />
      ) : (
        <PlatformHeader
      ...
      <Routes>
        <Route
          index
          element={<Home settings={settings} isCustomDomain={isCustomDomain} />}
        />
        <Route
          path="home"
          element={<Home settings={settings} isCustomDomain={isCustomDomain} />}
        ...
        <Route
          ...
          element={
            <ProductPage settings={settings} isCustomDomain={isCustomDomain} />
          }
        />
        <Route
          path="pages/products"
          element={
            <ProductsPage settings={settings} isCustomDomain={isCustomDomain} />
          }
        />
        <Route
        ...
        <Route
          path="cart"
          element={<Cart settings={settings} isCustomDomain={isCustomDomain} />}
        />
